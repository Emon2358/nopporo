...
def mount_and_copy(loop_dev, mount_dir, src_file, dst_filename):
    # Attempt to mount the loop device using different filesystem types.
    for fs_type in ["vfat", "msdos"]:
        print(f"Trying to mount {loop_dev} on {mount_dir} as {fs_type}...")
        cmd = ["sudo", "mount", "-o", "loop", "-t", fs_type, loop_dev, mount_dir]
